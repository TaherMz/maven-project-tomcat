<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <title>Welcome to our new Maven web application </title>
    <link rel="stylesheet" href="http://www.learntek.org/wp-content/uploads/2017/08/jenkins_image.png">
</head>
<body>
  <div class="jumbotron" style="background-color:white">
     <h1 class="text-center">Welcome to our Maven web Application AAAAAAA </h1>
      <h1> DevOps CI/CD Pipeline 07/12/2023 <h1>
      <img src="https://www.tek-up.de/plans/img/logo-header.png" alt="Spidertocat"
           class="img-responsive center-block" style={{width: 300px}}/>
      <h1 class="text-center">My Web Server deployment is done with Jenkins</h1>
      <h2 class="text-center">This project is auto-deployed by SCM Webhook trigger with using NGROK on Tomcat Server </h2>
  </div>
</body>
</html>
